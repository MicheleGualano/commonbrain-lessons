...
echo "[verified-lint]"
assert 0 "no verified -> ok"     python3 "$SCRIPTS/verified_field_lint.py" "$FX/gate-clean-sample.json"
assert 1 "verified:true -> fail" python3 "$SCRIPTS/verified_field_lint.py" "$FX/verified-asserted-sample.json"
